...
without needing to know the current job ID.
"""
...
def _sacct_states(job_id: str) -> dict:
    ...
    try:
        ...
        print(f"[WARN] sacct query failed ({e}); SLURM state column omitted.", file=sys.stderr)
        return {}
...
def main(design_matrix_path: str, data_root: str, job_id: str = None):
    design_matrix = pd.read_csv(design_matrix_path)
    slurm_states = _sacct_states(job_id) if job_id else {}
    ...
    for _, row in design_matrix.iterrows():
        ...
        state = slurm_states.get(row_index, 'unknown' if job_id else '')
        rows.append({
            'row_index': row_index, 'scenario_id': scenario_id, 'replicate_id': replicate_id,
            'stage': stage, 'slurm_state': state, 'total_elapsed_sec': total_elapsed_sec,
        })
...
if __name__ == '__main__':
    ...
    parser.add_argument('--job_id', default=None,
                         help="SLURM array job ID from submit_all.sh's "
                              "'Submitted 01_run.sh: <id>' line. Enables the SLURM "
                              "state column and failure-detection section.")
    args = parser.parse_args()
    main(args.design_matrix, args.data_root, args.job_id)
