...
\usepackage{graphicx}
%% Plate-notation figures live in the paper's Figures directory (path relative to this .tex).
%% Filenames contain spaces; a TeXLive >= 2019 kernel handles these directly.
\graphicspath{{../../../../bayesDREAM_paper/Figures/Plate Notation/}}
...
\begin{align}
  ...
  \hat\mu_t^{\log}    &= \log\bar y_t^{(0)} - \tfrac{1}{2}(\hat\sigma_t^{\log})^2.
    \label{eq:ntc_mu_log}
\end{align}
The full generative model is shown in Figure~\ref{fig:plate_ntc_negbinom}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.33\textwidth]{plate diagrams/fit_ntc NegBin.pdf}
  \caption[NTC model: negative binomial]{Plate diagram for the negative-binomial technical model
    (Equations~\ref{eq:ntc_negbinom_obs}--\ref{eq:ntc_mu_log}).
    Throughout, shaded nodes are observed data or fixed inputs, unshaded circles are
    latent random variables, and squares denote deterministic transforms.
    The baseline mean $\mu_t$ and inverse-overdispersion $o_t$ (with $\phi_t = 1/o_t^2$)
    are per-feature; the multiplicative technical correction $\alpha_{c,t}$ is per
    feature and technical group; $s_i$ is the size factor.
    The containment-prior rate $\beta_o$ is a single hyperparameter shared across all
    $T$ features and therefore lies outside the $T$ plate.}
  \label{fig:plate_ntc_negbinom}
\end{figure}
...
the effective concentration $\kappa_t = \min(\max(n_t^{(0)},\, 20),\, 200)$
...
\end{equation}
The plate diagram for the binomial technical model is shown in Figure~\ref{fig:plate_ntc_binom}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.20\textwidth]{plate diagrams/fit_ntc Binom.pdf}
  \caption[NTC model: binomial]{Plate diagram for the binomial technical model
    (Equations~\ref{eq:ntc_binom_obs}--\ref{eq:ntc_binom_alpha}).
    The baseline PSI $\mu_t$ (here $\mu_t^{\mathrm{psi}}$) and the additive logit-scale
    correction $\alpha_{c,t}$ (here $\alpha_{c,t}^{\mathrm{add}}$) generate the observed
    counts $y_{i,t}^{\mathrm{obs}}$ given the observed read-depth denominator $n_{i,t}$.}
  \label{fig:plate_ntc_binom}
\end{figure}
...
The plate diagram for the multinomial technical model is shown in Figure~\ref{fig:plate_ntc_multinomial}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.20\textwidth]{plate diagrams/fit_ntc Multinomial.pdf}
  \caption[NTC model: multinomial]{Plate diagram for the multinomial technical model
    (Equations~\ref{eq:ntc_dirichlet}--\ref{eq:ntc_multi_alpha}).
    The baseline category-probability vector $\mathbf{p}_t$ and the per-category additive
    logit corrections $\alpha_{c,t,k}^{\mathrm{logit}}$ (inner $K$ plate) generate the
    observed category-count vector $\mathbf{y}_{i,t}^{\mathrm{obs}}$ given the total count
    $n_{i,t}$.}
  \label{fig:plate_ntc_multinomial}
\end{figure}

\subsection{Normal and Student-\texorpdfstring{$t$}{t} models}
...
The plate diagram for the normal and Student-$t$ technical models is shown in Figure~\ref{fig:plate_ntc_normal}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.20\textwidth]{plate diagrams/fit_ntc Normal StudentT.pdf}
  \caption[NTC model: normal and Student-$t$]{Plate diagram for the normal and Student-$t$ technical models (Section~2.5).
    The per-feature mean $\mu_t$, additive technical correction $\alpha_{c,t}$
    (here $\alpha_{c,t}^{\mathrm{add}}$), and noise scale $\sigma_t$ generate the observed
    continuous value $y_{i,t}^{\mathrm{obs}}$. The Student-$t$ model is identical with
    fixed degrees of freedom $\nu = 3$.}
  \label{fig:plate_ntc_normal}
\end{figure}
...
\paragraph{Variational guide selection.}
\texttt{fit\_ntc} fits all likelihood families with a mean-field \texttt{AutoNormal}
variational guide by default, independent of $T$ or the compute device.
A more expressive alternative, \texttt{AutoIAFNormal}~\cite{bingham2019pyro}—a single
inverse-autoregressive-flow transform that jointly parameterises all latent
variables—is available for negative-binomial and other non-multinomial modalities via
an opt-in flag, and is best suited to small, deeply sequenced panels in which no
feature has near-zero NTC counts.
This restriction reflects a numerical instability specific to jointly-coupled guides.
In the negative-binomial observation model (equation~\ref{eq:ntc_negbinom_obs}), the
natural-parameter (logit) computation involves $\log(\mu_{i,t} + \epsilon)$ for a small
numerical-stability constant $\epsilon$; its derivative $1/(\mu_{i,t} + \epsilon)$ grows
without bound as the size-factor-adjusted mean $\mu_{i,t} = \mu_t\,\alpha_{c_i,t}\,s_i
\to 0$, which occurs whenever a feature has very low NTC counts (e.g.\ a handful of
total reads across all NTC cells) together with a low-size-factor cell—an expected
occurrence in realistically sparse single-cell data, not a data-quality defect.
Under \texttt{AutoIAFNormal}, a single autoregressive network parameterises the joint
posterior over all $T$ features simultaneously, so one such feature's exploding
gradient can, after a single optimiser step, corrupt the transform's shared weights and
hence every feature's sample—observed empirically as the entire fit diverging to
\texttt{NaN} within the first few SVI iterations.
\texttt{AutoNormal}'s per-feature factorisation is far less susceptible to this failure
mode: a poorly-conditioned gradient for one feature does not propagate through
parameters shared with the others.
...
\begin{align}
  \alpha_\sigma &\sim \mathrm{Exponential}(r_\alpha)
      \label{eq:cis_alpha_sigma} \\
  \beta_\sigma  &\sim \mathrm{Exponential}(r_\beta)
      \label{eq:cis_beta_sigma} \\
  \sigma_{\mathrm{eff},g} &\iid \mathrm{Gamma}(\alpha_\sigma, \beta_\sigma)
...
The full generative model for the low-MOI cis fit is shown in Figure~\ref{fig:plate_cis_lowmoi}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.21\textwidth]{plate diagrams/fit_cis lowMOI.pdf}
  \caption[Cis model (low-MOI)]{Plate diagram for the cis model in the default low-MOI setting
    (Equations~\ref{eq:cis_pop_mu}--\ref{eq:cis_obs}).
    The population parameters $\mu,\sigma$ and per-guide standardised noise
    $\varepsilon_g$ deterministically define the guide-effect $x_g^{\mathrm{eff}}$
    (square); the hierarchical within-guide noise $\sigma_g^{\mathrm{eff}}$
    (with hyperparameters $\alpha_\sigma,\beta_\sigma$) then generates the per-cell latent
    expression $x_i$. Observed counts $x_i^{\mathrm{obs}}$ follow a negative binomial with
    fixed inputs $\alpha_c^x$ (transferred from \texttt{fit\_ntc}), size factor $s_i$, and
    dispersion $\phi_x$ (shaded, held fixed at the \texttt{fit\_ntc} estimate).
    \textit{In the high-MOI extension} (Section~\ref{sec:highmoi}), the single guide
    index $g_i$ is replaced by a binary membership row $W_{i\cdot}$ from the
    guide-assignment matrix $W\in\{0,1\}^{N\times G}$. The edge from the guide plate to
    the cell plate becomes a sum: $\log_2 x_{\mathrm{mean},i} = \sum_g W_{ig}\log_2
    x_g^{\mathrm{eff}} - (k_i-1)\log_2\bar x^{\mathrm{NTC}}$, where $k_i = \sum_g W_{ig}$
    is the number of guides in cell $i$ and $\bar x^{\mathrm{NTC}}$ is a
    precision-weighted mean of NTC guide effects. The per-cell noise $\sigma_{\mathrm{eff},g_i}$
    likewise becomes a precision-weighted combination over all $k_i$ active guides. The
    observation model and all other nodes are unchanged.}
  \label{fig:plate_cis_lowmoi}
\end{figure}
...
\subsection{Baseline parameter \texorpdfstring{$A$}{A}}
\label{sec:trans_A}

The parameter $A_t$ is the asymptotic intercept of the dose-response curve: the value
$f_t$ approaches when all Hill terms contribute zero.
For a single activating component ($n_{t} > 0$) this limit is reached as
$\hat x_i \to 0$; for a repressing component ($n_{t} < 0$) as $\hat x_i \to \infty$.
In the additive Hill model, where the two components may have opposite signs, neither
limit recovers $A_t$ exactly, and $A_t$ is best understood as the shared intercept
of the sum.
When the Hill curve is flat (the model fits no dependence), $A_t$ is the sole active parameter
and equals the feature level across all cis-expression values.
Its prior is distribution-specific, noise-adaptive, and anchored near the NTC expression
level, which is the prior if the feature is a null gene (i.e. no dependence).
...
\subsection{Sparsity gate \texorpdfstring{$\alpha_t$}{alpha\_t}}
\label{sec:sparsity}
...
allow the model to capture biphasic dose-response curves (e.g.\ step-wise monotonic curves,
non-monotonic curves, and other more complex curvature shapes).

\subsection{Priors on Hill parameters}

Because the cis gene is always modelled with a negative-binomial observation model
(Section~\ref{sec:cis_fitting}), the cis expression $\hat x_i$ is always on the same
absolute count scale regardless of which likelihood is used for trans features.
Consequently, the EC$_{50}$ parameter $K$ and the Hill coefficient $n$ are
\emph{distribution-agnostic}: $K$ sets a threshold on the $x$-axis (cis expression),
and $n$ controls the steepness of the sigmoid on that axis, neither of which depends on
the scale of the trans observations.
Only the amplitude $V$ is distribution-specific, because it is measured on the $y$-axis
(trans feature scale) and must be expressed in units appropriate to each likelihood family
(counts for negbinom, probability for binomial/multinomial, natural units for normal/studentt).
...
(though the user can choose this to be upper, 95th percentile of per-guide $x_true$ means, or lower, 5th percentile of per-guide $x_true$ means, end of the expression range instead).
...
\begin{equation}
  n_{a,t}^{\mathrm{raw}} \sim \mathcal{N}(\tilde n_\mu^{\mathrm{raw}}, \sigma_{n_a,t}),
  \quad
  n_{a,t} = h(n_{a,t}^{\mathrm{raw}};\; n_{\min}, n_{\max}),
\end{equation}
where $h(\cdot)$ is a differentiable soft-clamp (tanh-based) mapping to $(n_{\min}, n_{\max})$.
The scale $\sigma_{n_a,t} \sim \mathrm{Exponential}(1)$ is sampled per feature to allow
heterogeneous regularisation, and $\tilde n_\mu^{\mathrm{raw}}$ is the \emph{prior mean of}
$n_{a,t}^{\mathrm{raw}}$ (not the same as $n_{a,t}^{\mathrm{raw}}$ itself, which varies
across features and samples).
For the additive Hill model, the Hill b component uses an independent prior with the same
structure:
\begin{equation*}
  n_{b,t}^{\mathrm{raw}} \sim \mathcal{N}(\tilde n_\mu^{\mathrm{raw}},\, \sigma_{n_b,t}),
  \quad \sigma_{n_b,t} \sim \mathrm{Exponential}(1),
\end{equation*}
with the same soft-clamp and bounds applied.
%
...
for asymmetric bounds the correction shifts the unconstrained mean to compensate so the
constrained mode remains at $n_\mu = 0$.

The bounds $n_{\min}$ and $n_{\max}$ are computed from the observed range of $\hat x_i$
to prevent float32 overflow in $\hat x^n$:
\begin{align}
  n_{\min} &= -\log(\texttt{float32\_max})\,/\,|\log \hat x_{\min}|
    \quad \text{if } \hat x_{\min} < 1, \text{ else } -100, \\
  n_{\max} &= \phantom{-}\log(\texttt{float32\_max})\,/\,|\log \hat x_{\max}|
    \quad \text{if } \hat x_{\max} > 1, \text{ else } +100,
\end{align}
where $\hat x_{\min}$ and $\hat x_{\max}$ are the minimum and maximum observed cis
expression values.
The fallback values $\pm 100$ apply when the observed expression range does not cross
the value 1 on the relevant side (e.g.\ $n_{\min} = -100$ when all cells have
$\hat x_i \ge 1$, making large negative $n$ safe from overflow).
...
\end{equation}
The plate diagram for the negative-binomial trans model is shown in Figure~\ref{fig:plate_trans_negbinom}.

\begin{figure}[htbp]
  \centering
  \includegraphics[height=0.35\textheight]{plate diagrams/fit_trans NegBin.pdf}
  \caption[Trans model: negative binomial]{Plate diagram for the negative-binomial trans model
    (Equation~\ref{eq:trans_nb_lik}).
    The dose-response value $f_t(\hat x_i)$ (square) is assembled from the baseline $A_t$,
    the two sparsity gates $\alpha_t,\beta_t$, and the two Hill components
    $(V_{a,t},K_{a,t},n_{a,t})$ and $(V_{b,t},K_{b,t},n_{b,t})$, with each Hill coefficient
    the soft-clamp of a raw draw $n_{\cdot,t}^{\mathrm{raw}}\!\sim\!\mathcal{N}(\cdot,
    \sigma_{n_\cdot,t})$. The cis expression $\hat x_i$, technical correction
    $\hat\alpha_{c,t}$, size factor $s_i$, and NTC overdispersion $\hat o_t^{\mathrm{NTC}}$
    (anchoring the $A_t$ prior) are fixed inputs. As in \texttt{fit\_ntc}, $\beta_o$ is
    shared across all $T$ features.}
  \label{fig:plate_trans_negbinom}
\end{figure}
...
\end{equation}
The plate diagram for the binomial trans model is shown in Figure~\ref{fig:plate_trans_binom}.

\begin{figure}[htbp]
  \centering
  \includegraphics[height=0.35\textheight]{plate diagrams/fit_trans Binomial.pdf}
  \caption[Trans model: binomial]{Plate diagram for the binomial trans model
    (Equation~\ref{eq:trans_binom_lik}).
    As in Figure~\ref{fig:plate_trans_negbinom}, $f_t(\hat x_i)$ is built from $A_t$, the
    gates $\alpha_t,\beta_t$, and the two Hill components. No variance parameter is
    estimated: the observation is fully determined by the read-depth denominator $n_{i,t}$,
    together with the fixed cis expression $\hat x_i$ and additive technical correction
    $\hat\alpha_{c,t}$.}
  \label{fig:plate_trans_binom}
\end{figure}
...
The plate diagram for the multinomial trans model is shown in Figure~\ref{fig:plate_trans_multinomial}.

\begin{figure}[htbp]
  \centering
  \includegraphics[height=0.36\textheight]{plate diagrams/fit_trans Multinomial.pdf}
  \caption[Trans model: multinomial]{Plate diagram for the multinomial trans model
    (Equation~\ref{eq:trans_multi_lik}).
    The dose-response vector $\mathbf{f}_t(\hat x_i)$ (square) is constructed per category:
    the baseline $\mathbf{A}_{t,k}$ spans all $K$ categories (inner $K$ plate), while the
    two Hill components and their gates $\alpha_{t,k},\beta_{t,k}$ act on the $K-1$ free
    categories (residual-category construction; outer $K{-}1$ plate).
    Fixed inputs are the cis expression $\hat x_i$, per-category logit corrections
    $\hat\alpha_{c,t,k}^{\mathrm{logit}}$, and the total count $n_{i,t}$.}
  \label{fig:plate_trans_multinomial}
\end{figure}
...
The plate diagram for the normal and Student-$t$ trans models is shown in Figure~\ref{fig:plate_trans_normal}.

\begin{figure}[htbp]
  \centering
  \includegraphics[height=0.39\textheight]{plate diagrams/fit_trans Normal StudentT.pdf}
  \caption[Trans model: normal and Student-$t$]{Plate diagram for the normal and Student-$t$ trans models
    (Equations~\ref{eq:trans_normal_lik} and the Student-$t$ variant).
    As in Figure~\ref{fig:plate_trans_negbinom}, $f_t(\hat x_i)$ is assembled from $A_t$,
    the gates $\alpha_t,\beta_t$, and the two Hill components; the per-feature noise scale
    $\sigma_t$ enters the likelihood directly. The NTC noise scale
    $\hat\sigma_t^{\mathrm{NTC}}$ anchors the $A_t$ prior. The Student-$t$ model is
    identical with fixed $\nu_t = 3$.}
  \label{fig:plate_trans_normal}
\end{figure}
...
\paragraph{Two-stage inference for \texttt{additive\_hill}.}

Fitting the additive Hill model directly from random initialisation risks the two
Hill components converging to redundant solutions (e.g.\ both capturing the same
inflection) or the second component absorbing noise.
To avoid this, inference for \texttt{additive\_hill} proceeds in two phases.

\textit{Phase 1 (single-Hill warmup).}
A single-Hill model (Equation~\ref{eq:single_hill}) is fit for
$\sim\!44\%$ of the total training budget (temperatures cooling from $1.0$ to $0.5$;
see Section~\ref{sec:sparsity} for the temperature schedule).
At the high temperatures used in this phase the relaxed-Bernoulli sparsity gate $\alpha_t$
is relatively soft, providing only limited regularisation: the goal is to find the
best single-Hill explanation of the data, not to suppress weak effects.

\textit{Phase 2 (additive fit).}
The second component $(V_{b,t}, K_{b,t}, n_{b,t})$ and its sparsity gate $\beta_t$ are
introduced, initialised from the Phase~1 parameter store.
Because the first Hill component already captures the dominant dose-response shape,
the second component starts near zero and must earn its place against the strong
sparsity prior ($p_0 = 10^{-6}$, $\tau$ annealed from $1.0$ to $0.1$).
Genes for which a single Hill provides an adequate fit will have $\beta_t \approx 0$
in the posterior; the second Hill is retained only when it substantially improves
the fit (e.g.\ biphasic or non-monotonic dose-response curves).
