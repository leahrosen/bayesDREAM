...
Three visually distinct false-positive populations:
...
1. **Broad "a significant" population** (green, log2 NTC expression ≈ −5 to −10): genes with
   ...
   not to true dose-response signal.
...
### 4. NTC cells carry other perturbations (high MOI confounding)
...
Genes with very low NTC expression are the most susceptible: small absolute residuals
translate to large fold-changes, so even small sum_factor leakage produces significant fits.
These are the "a significant" green population in Image #3.
...
**Anchor o_x in the cis model to the technical fit** *(most targeted)*  
